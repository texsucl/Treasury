...
//發票年月檢核    add by daiyu 20180130
function checkInvoYm(str, require) {
    if (!str) {
        if (require)
            return false;
        else
    ...
    if (str.length != 5)
        ...
        return checkTwYearMonth("0" + str, true);

};

//發票字軌號碼檢核    add by daiyu 20180130
function checkInvoTrackNo(str, require) {
    if (!str) {
        if (require)
            return false;
        else
    ...
    if (str.length != 10)
        ...
        if (checkBEng(str.substr(0, 2))) {
            if (checkNumP(str.substr(2, 8)))
                return true;
            else
                return false;
        } else {
