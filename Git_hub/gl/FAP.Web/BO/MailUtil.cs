...
namespace FAP.Web.BO
{
    public class MailUtil
    {
        ...
        private Dictionary<string, UserBossModel> qryEmp(string usrId, Dictionary<string, UserBossModel> empMap
            , EacConnection con, OaEmpDao oaEmpDao)
        {
            if (!empMap.ContainsKey(usrId))
            {
                UserBossModel userBossModel = oaEmpDao.getEmpBoss(usrId, con);
        ...
        public bool sendMail(string[] MailTos, string MailSub, string MailBody, bool isBodyHtml
            ...
            try
            {
                ...
                string devRec = System.Configuration.ConfigurationManager.AppSettings.Get("devRec");    //add by daiyu 20210208
                ...
                if (MailTos != null)
                {
                    for (int i = 0; i < MailTos.Length; i++)
                    {
                        if (!string.IsNullOrEmpty(MailTos[i].Trim()))
                        {
                            ...
                            {
                                mms.To.Add(new MailAddress(devRec));    //modify by daiyu 20210208
        ...
        /// <summary>
        /// 寄MAIL，且寫LOG，使其可在"ORTB008 MAIL寄送紀錄查詢"看到
        /// </summary>
        ...
        /// <param name="deleteFileAttachment"></param>
        /// <param name="bWriteLog"></param>
        /// <param name="logKey"></param>
        /// <returns></returns>
        public bool sendMailMultiRTLog(List<UserBossModel> userBoss, string MailSub, string MailBody, bool isBodyHtml
          , string mailAccount, string mailPwd
          , string[] filePaths, bool deleteFileAttachment
          , bool bWriteLog
          , string logKey)
                ...
                using (SqlConnection conn = new SqlConnection(strConn))
                {
                    ...
                    foreach (UserBossModel d in userBoss)
                    {
                        try
                        {
                            ...
                            frtMailLog.MAIL_RESULT = mailResult;
                            frtMailLog.RESULT_DESC = logKey + (StringUtil.toString(resultDesc) == "" ? "" : ":" + resultDesc);
                            frtMailLog.MAIL_SUB = mailSub;
