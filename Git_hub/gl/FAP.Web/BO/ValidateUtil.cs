...
namespace FAP.Web.BO
{
    public class ValidateUtil
    {
        ...
        /// <summary>
        /// 檢核是否為符合民國年格式(YYYYMMDD)
        /// </summary>
        /// <param name="input"></param>
        /// <param name="inputCol"></param>
        /// <returns></returns>
        public bool chkChtDate(string input)
        {
            bool bPass = true;

            try
            {
                input = StringUtil.toString(input).PadLeft(8, '0');
                Convert.ToDateTime((Convert.ToInt16(input.Substring(0, 4)) + 1911).ToString() + "/" + input.Substring(4, 2) + "/" + input.Substring(6, 2));
            }
            catch (Exception e)
            {
                bPass = false;
