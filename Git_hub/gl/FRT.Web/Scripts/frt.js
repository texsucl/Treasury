...
//右補字串
function rightPad(val, length, letter) {
    var str = '' + val;
    while (str.length < length) {
        str = str + letter;
    }
...
//時分檢核
function checkTime(str, require) {
    if (!str) {
        if (require)
            return false;
        else
    ...
    str = leftPad(String(str), 4, '0')

    var hh = str.substr(0, 2),
        mi = str.substr(2, 2)

    if ((hh >= "00" && hh <= "23") & (mi >= "00" && mi <= "59"))
        return true;
    else 
        return false;
}
...
//民國日期檢核
function checkDateTW(str, require) {
    if (!str) {
        if (require)
            return false;
        else
    ...
    var y = parseInt(str.substr(0, 4)) + 1911,
        m = str.substr(4, 2),
        d = str.substr(6, 2);
...
//檢核字串是否為數字(含小數)
function checkDecimal(str, intLen, decLen) {
    //var regExp = new RegExp("/^[0-9]{" + intLen + "}+(.[0-9]{" + decLen + "})?$/");
    var regExp = /^[0-9]+(.[0-9]*)?$/
    ...
    if (regExp.test(str)){
        ...
        if (arrStr.length == 2) {
            if (arrStr[1].length > decLen)
                return false;
        }

        if (arrStr[0].length > intLen)
            return false;
        else
            return true;
