...
namespace FRT.Web.Daos
{
    public class FRTXmlREachDao
    {
        ...
        public List<ORTB012Model> qryForORTB012(dbFGLEntities db, string actDateB, string actDateE, string fastNoB, string fastNoE)
        {
            ...
            rows.AddRange(db.FRT_XML_R_eACH.AsNoTracking().Where(
        ...
        public FRT_XML_R_eACH qryForORTB009(string fastNo)
        {
            using (dbFGLEntities db = new dbFGLEntities())
            {
                FRT_XML_R_eACH d = db.FRT_XML_R_eACH.Where(x => x.FAST_NO == fastNo).OrderByDescending(x => x.CRT_TIME).FirstOrDefault();
                return d;
            }
