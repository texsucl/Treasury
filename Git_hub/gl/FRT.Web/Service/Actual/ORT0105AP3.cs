...
{
    public class ORT0105AP3 : IORT0105Report
    {
        ...
        public MSGReturnModel<Tuple<List<ORT0105AP2Model>, List<ORT0105AP2Model>>> getData(FRT_CROSS_SYSTEM_CHECK schedulerModel, string date_s = null, string date_e = null)
        {
            ...
            List<ORT0105AP2Model> RVMNData = new List<ORT0105AP2Model>();
...
REMIT_AMT AS AMT,
FAIL_DATE AS FAIL_DATE 
from FRTRVMN0
where FAIL_DATE between :DATE_S_1 and :DATE_E_1
...
";
                        ...
                        while (dbresult.Read())
                        {
                            ...
                            model.REMIT_DATE = dbresult["REMIT_DATE"]?.ToString()?.Trim()?.TWDateToDate(); //基準日期
                            model.UPD_DATE = dbresult["UPD_DATE"]?.ToString()?.Trim()?.TWDateToDate(); //異動日期
                            model.AMT = dbresult["AMT"]?.ToString().stringToDecimalN();  //金額
                            model.FAIL_DATE = dbresult["FAIL_DATE"]?.ToString()?.Trim()?.TWDateToDate(); //退匯日期
                            RVMNData.Add(model);
                        }
                        com.Dispose();
                    }
...
REMIT_MONEY_NET_AMOUNT AS AMT,
CURR_CODE_1 AS CURR,
CONVERT(varchar, REMIT_MONEY_DATE, 23) AS REMIT_DATE,
CONVERT(varchar, VOUCHER_DATE_ACC02010101, 23) AS UPD_DATE,
CONVERT(varchar, VOUCHER_DATE, 23) AS FAIL_DATE
FROM GLSIACT..ACC03050301 with(nolock)
WHERE VOUCHER_DATE between @DATE_S_2 and @DATE_E_2
AND FROM_REMIT_BANK_SERIAL_NO not like 'Q%'
AND FROM_REMIT_BANK_SERIAL_NO not like 'W%'
AND isnull(DEPT_BOSS_ALLOW_YN,'') = 'Y'
...
";
                ...
                result.Datas = new Tuple<List<ORT0105AP2Model>, List<ORT0105AP2Model>>(RVMNData, WanpieData);
            }
            catch (Exception ex)
            {
        ...
        public MSGReturnModel<ORT0105AP2ReportModel> getReportData(Tuple<List<ORT0105AP2Model>, List<ORT0105AP2Model>> datas)
        {
            ...
            ORT0105AP2ReportModel _ORT0105AP2ReportModel = new ORT0105AP2ReportModel();
            ...
            try
            {
                ...
                {
                    var _rvmn_all = datas.Item1.Where(x => x.CURR == _curr).ToList();  //退匯主檔(FRTRVMN0)
                    var _wanpie_all = datas.Item2.Where(x => x.CURR == _curr).ToList(); //Wanpie匯款檔(匯款單多來源)
                    var _main_c = _rvmn_all.Count;
                    var _wanpie_c = _wanpie_all.Count;
                    var _main_amt = _rvmn_all.Where(x => x.AMT != null).Sum(x => x.AMT) ?? 0m;
                    ...
                    {
                        decimal d = 0m;
                        foreach (var _FEE_SEQ in _rvmn_all.Select(x => x.FEE_SEQ).Distinct()
                              .Union(_wanpie_all.Select(x => x.FEE_SEQ).Distinct()))
                        {
                            var _main = _rvmn_all.FirstOrDefault(x => x.FEE_SEQ == _FEE_SEQ);
                                ...
                                (_main.AMT != _wanpie.AMT))
                            {
                                subModel.Add(new ORTReportBDetailModel()
                                {
                                    ...
                                    Date_2_L = _main?.FAIL_DATE ?? "-", //退匯日期 (左邊)
                                    AMT_L = _main?.AMT?.formateThousand() ?? "-", //金額 (左邊)
                                    Date_1_R = _wanpie?.REMIT_DATE ?? "-", //匯款日期 (右邊)
                                    Date_2_R = _wanpie?.FAIL_DATE ?? "-", //退匯日期 (右邊)
                ...
                foreach (var item in groupSubData.OrderBy(x => x.Item1))
                {
                    if (_first)
                    {
                        ...
                        {
                            FEE_SEQ = "差異總計 :",
                            CURR = $@"{subModel.Where(x => x.CURR == item.Item1).Count().ToString().formateThousand()} 筆",
                            Date_1_L = "總計金額 :",
                            Date_2_L = $@"{item.Item1}",
                ...
                #endregion
                result.RETURN_FLAG = true;
                _ORT0105AP2ReportModel.model = new Tuple<List<ORTReportBModel>, bool, List<ORTReportBDetailModel>>(mainModel, subReportCheck, subModel_fix);
                result.Datas = _ORT0105AP2ReportModel;
            }
            catch (Exception ex)
            {
                var _msg = ex?.ToString();
